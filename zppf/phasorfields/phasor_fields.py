...
def __reconstruct_pf_plane(RSD_prop, wavelengths, coords, xl, fH, freq_weights,
                           image, grid_z, confocal_capture = False,
                           analysis = {}, idx_z = 0 ):
    z = grid_z[idx_z]
    kernels = RSD_prop.get(z, wavelengths)
    if confocal_capture:
        # Apply the convolution with the RSD kernels twice
        fV = np.fft.fft2(fH, s = (2*fH.shape[1]-1, 2*fH.shape[2]-1),
                            axes = (-2,-1))\
            * np.fft.fft2(kernels**2, axes = (-2,-1))
        # Return the propagation in time 0
        image[idx_z] = np.sum(np.fft.ifft2(fV, axes = (-2,-1))\
                                [..., -fH.shape[1]:, -fH.shape[2]:]\
                    *freq_weights.reshape(-1, 1, 1), axis = 0)
    else:
        # Apply the convolution with the RSD kernels
        fV = np.fft.fft2(fH, s = (2*fH.shape[1]-1, 2*fH.shape[2]-1),
                            axes = (-2,-1))\
            * np.fft.fft2(kernels, axes = (-2,-1))
        # Apply the light propagation and return the reconstruction at time 0
        light_prop = point_to_point_propagator(coords[idx_z], xl, wavelengths)
        image[idx_z] = np.sum(np.fft.ifft2(fV, axes = (-2,-1))\
                                        [..., -fH.shape[1]:, -fH.shape[2]:]\
                            *freq_weights.reshape(-1, 1, 1)*light_prop, axis = 0)
...
def __reconstruct_pf_plane_by_point(grid_coords, rec_coords, wavelengths, xl,
                                    ...
                                    confocal_capture=False, analysis = {},
                                    idx_z = 0 ):
...
def phasor_fields_reconstruction(data, central_wavelength, n_pulses, z_begin,
    ...
    confocal_capture = data.is_confocal()
    ...
    if PF_by_point:
        plane_rec_func = partial(__reconstruct_pf_plane_by_point,
                                 ...
                                 confocal_capture, analysis_concurrent)
    else:
